...
Computational analysis depends on whether a reference genome for the species being investigated is available. Given a reference genome, the key to **"resequence"** is to find the best match between each short read and the refernce sequence (for example, the billions of bases of the human genome). However, this method of NGS resequencing allows for unwanted variations, including nucleotide mismatches and mutations (insertions or deletions) -- in other words, Ant Man, were he processed by NGS, may not be Ant Man anymore when "he" is fully "comes out". Then, when a reference genome is _not_ available, **"reference free"** assembly (the de novo assembly) must be imployed to build long sequences by using overlaps among short reads. Reference free assembly is a whole different, difficult, ballpark. 

As a whole, the de novo assembly of the human genome takes about [48](https://www.nature.com/articles/s41598-021-88321-5) hours on a supercomputer -- but I would not be able to guarantee that the human genome will come out fully accurate and with an efficiency comparable to how a future quantum computering method may be able to do it. 

**Cue Quantum Computing**

Quantum algorithms are being developed to save the day, promising to overcome NGS as the next "next generation sequencing." These algorithms look to pattern match to solve for the inaccuracies with resequencing. [Grover's search algorithm](https://quantum-computing.ibm.com/composer/docs/iqx/guide/grovers-algorithm) has shown promise in solving the problem of biological sequence alighment and may address the problems of repeated sequences and non exact matches (such as mutations or mismatches). QiBAM is the sole algorithm developed specifically for solving problems with resequencing, which is an extension of Grover's search algorithm and allows for errors in read and reference allignments to be stores in quantum memory (QRAM). Broadly, there are also quantum solutions being developed to address the problems with de nove assembly (reference free). 

Alas, the information I have found regarding how Quantum computing may one day be harnessed to process Ant Man's genome proves quite difficult to extrapolate from regarding how quickly this sequencing would occur. However, this response to the original question posed by 1-week-younger Naomi is the manifestation of 1-week-older Naomi's efforts to seek unconventional solutions to unconventional problems -- 1-week-older Naomi has devoured information about quantum computing, and so she is proud.  

### Quick Reflection
Thank you for coming along with me on this twisting, winding journey on the topic of the intersection between fantastical superheroes and quantum computers. While I started by trying to answer the question, "how long would it take for a quantum computer to process (sequence) Ant Man," I soon understood that I had to dive deeper into what DNA sequencing actually means. I got to learn about ways to equate information stored in DNA with bits (and even qubits), brush up on my altair knowledge, and educate myself on the future of DNA sequencing through the very application of quantum computing. This presentation of data and knowledge is purely exploratory, and I hope that you've enjoyed your itme with me!!
